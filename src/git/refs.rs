...
pub fn parse_refs(bytes: &[u8]) -> Vec<BranchInfo> {
    ...
    for raw in text.split('\n') {
        ...
        out.push(BranchInfo {
            ...
            oid: fields.next().unwrap_or("").to_string(),
            upstream: non_empty(fields.next()),
            committer_date: non_empty(fields.next()),
            subject: non_empty(fields.next()),
            worktree_path: non_empty(fields.next()),
        });
    }
    out
}
